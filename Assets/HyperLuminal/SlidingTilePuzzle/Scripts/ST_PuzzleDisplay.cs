...
public class ST_PuzzleDisplay : MonoBehaviour 
{
	...
	{
		new Vector2(2,0), // Target position for tile 0
		new Vector2(1,0),  // Target position for tile 1 #CORRECT
		new Vector2(0,0),  // Target position for tile 2
		new Vector2(2,1), // Target position for tile 3
		new Vector2(1,1),  // Target position for tile 4
		new Vector2(0,1), // Target position for tile 5
		new Vector2(2,2),  // Target position for tile 6
		new Vector2(1,2),  // Target position for tile 7
		new Vector2(0,2),  // Target position for tile 8
	};
	...
	public Vector3 tileRotation = new Vector3(90.0f, 90.0f, 0.0f);
	...
	void Start () 
	{
		...
		SetInitialCorrectLocations();
	}
	private void SetInitialCorrectLocations()
	{
		for (int j = Height - 1; j >= 0; j--)
		{
			...
			{
				ST_PuzzleTile tile = TileDisplayArray[i, j].GetComponent<ST_PuzzleTile>();
				// Use the predefined target positions for setting the CorrectLocation property.
				tile.ArrayLocation = targetPositions[increment];
				increment++;
	...
	public IEnumerator CheckForComplete()
	{
		while (!Complete)
		{
			...
			//Debug.Log($"Puzzle Complete: {Complete}");
			yield return new WaitForSeconds(1.0f);
		}
		...
		if (Complete)
		{
			...
			//Trigger painting falling down animation
			paintingAnimation.Play("PaintingAnimation");
		}
